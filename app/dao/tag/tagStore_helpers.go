...
func (record *tag_Store) upgradeProcessing() error {
...
func (h *tag_Store) postGetProcessing() error {
...
func (record *tag_Store) preDeleteProcessing() error {
...
func tagClone(ctx context.Context, source tag_Store) (tag_Store, error) {
	//TODO: Add any clone processing here
	panic("Not Implemented")
	return tag_Store{}, nil
}

func tagJobProcessor(j jobs.Job) {
	...
	for tagEntryIndex, tagRecord := range tagEntries {
		logHandler.ServiceLogger.Printf("[%v] %v(%v/%v) %v", jobs.CodedName(j), domain, tagEntryIndex+1, notagEntries, tagRecord.Raw)
		tagRecord.UpdateWithAction(context.TODO(), audit.GRANT, "Job Processing")
		tagRecord.UpdateWithAction(context.TODO(), audit.SERVICE, "Job Processing "+j.Name())
		count++
	}
	clock.Stop(count)
}
