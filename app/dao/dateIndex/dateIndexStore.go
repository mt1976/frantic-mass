...
/// DO NOT CHANGE THIS FILE OTHER THAN TO RENAME "dateIndex" TO THE NAME OF THE DOMAIN ENTITY
/// DO NOT CHANGE THIS FILE OTHER THAN TO RENAME "dateIndex" TO THE NAME OF THE DOMAIN ENTITY
/// DO NOT CHANGE THIS FILE OTHER THAN TO RENAME "dateIndex" TO THE NAME OF THE DOMAIN ENTITY
...
func Count() (int, error) {
	...
	return activeDB.Count(&DateIndex{})
}
...
func GetById(id any) (DateIndex, error) {
	return GetBy(FIELD_ID, id)
}

func GetByKey(key any) (DateIndex, error) {
	return GetBy(FIELD_Key, key)
}

func GetBy(field string, value any) (DateIndex, error) {
	...
	if field == FIELD_ID && reflect.TypeOf(value).Name() != "int" {
		...
		return DateIndex{}, commonErrors.WrapDAOReadError(domain, field, value, fmt.Errorf(msg, value))
	}

	if err := dao.IsValidFieldInStruct(field, DateIndex{}); err != nil {
		return DateIndex{}, err
	}

	if err := dao.IsValidTypeForField(field, value, DateIndex{}); err != nil {
		return DateIndex{}, err
	}
	...
	if err := activeDB.Retrieve(field, value, &record); err != nil {
		clock.Stop(0)
		return DateIndex{}, commonErrors.WrapRecordNotFoundError(domain, field, fmt.Sprintf("%v", value))
	}

	if err := record.postGet(); err != nil {
		clock.Stop(0)
		return DateIndex{}, commonErrors.WrapDAOReadError(domain, field, value, err)
	}
...
func GetAll() ([]DateIndex, error) {

	dao.CheckDAOReadyState(domain, audit.GET, initialised) // Check the DAO has been initialised, Mandatory.

	recordList := []DateIndex{}

	clock := timing.Start(domain, actions.GETALL.GetCode(), "ALL")

	if errG := activeDB.GetAll(&recordList); errG != nil {
		clock.Stop(0)
		return []DateIndex{}, commonErrors.WrapNotFoundError(domain, errG)
	}
...
func GetAllWhere(field string, value any) ([]DateIndex, error) {
	dao.CheckDAOReadyState(domain, audit.GET, initialised) // Check the DAO has been initialised, Mandatory.

	recordList := []DateIndex{}
	resultList := []DateIndex{}
...
func (record *DateIndex) Spew() {
	logHandler.InfoLogger.Printf("[%v] Record=[%+v]", domain, record)
}

func (record *DateIndex) Validate() error {
	return record.validationProcessing()
}

func (record *DateIndex) Update(ctx context.Context, note string) error {
	return record.insertOrUpdate(ctx, note, actions.UPDATE.GetCode(), audit.UPDATE, actions.UPDATE.GetCode())
}

func (record *DateIndex) UpdateWithAction(ctx context.Context, auditAction audit.Action, note string) error {
	return record.insertOrUpdate(ctx, note, actions.UPDATE.GetCode(), auditAction, actions.UPDATE.GetCode())
}

func (record *DateIndex) Create(ctx context.Context, note string) error {
	return record.insertOrUpdate(ctx, note, actions.CREATE.GetCode(), audit.CREATE, actions.CREATE.GetCode())
}

func (record *DateIndex) Clone(ctx context.Context) (DateIndex, error) {
	return dateIndexClone(ctx, *record)
}

func (record *DateIndex) ExportRecordAsJSON(name string) {
...
func Drop() error {
	return activeDB.Drop(DateIndex{})
}
...
func ImportRecordsFromCSV() error {
	return importExportHelper.ImportCSV(domain, &DateIndex{}, tempalteImportProcessor)
}
