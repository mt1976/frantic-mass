...
func tempalteImportProcessor(inOriginal **DateIndex) (string, error) {
	//TODO: Build the import processing functionality for the dateIndex_Store data here
	//
	...
	// if err != nil {
	// 	logHandler.ImportLogger.Panicf("Error importing dateIndex: %v", err.Error())
	// }

	// Return the created entry and nil error
	//logHandler.ImportLogger.Printf("Imported %v [%+v]", domain, importedData)

	stringField1 := strconv.Itoa(importedData.Date.Year()) + "-" + strconv.Itoa(int(importedData.Date.Month())) + "-" + strconv.Itoa(importedData.Date.Day())

	_, err := Create(context.TODO(), importedData.Date)
	if err != nil {
