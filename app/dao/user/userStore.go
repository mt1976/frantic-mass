...
func GetById(id any) (User, error) {
	return GetBy(FIELD_ID, id)
}

func GetByKey(key any) (User, error) {
	return GetBy(FIELD_Key, key)
}

func GetBy(field string, value any) (User, error) {
	...
	if field == FIELD_ID && reflect.TypeOf(value).Name() != "int" {
		...
		return User{}, commonErrors.WrapDAOReadError(domain, field, value, fmt.Errorf(msg, value))
	}

	if err := dao.IsValidFieldInStruct(field, User{}); err != nil {
		return User{}, err
	}

	if err := dao.IsValidTypeForField(field, value, User{}); err != nil {
		return User{}, err
	}
	...
	if err := activeDB.Retrieve(field, value, &record); err != nil {
		clock.Stop(0)
		return User{}, commonErrors.WrapRecordNotFoundError(domain, field, fmt.Sprintf("%v", value))
	}

	if err := record.postGet(); err != nil {
		clock.Stop(0)
		return User{}, commonErrors.WrapDAOReadError(domain, field, value, err)
	}
...
func GetAll() ([]User, error) {
	...
	if errG := activeDB.GetAll(&recordList); errG != nil {
		clock.Stop(0)
		return []User{}, commonErrors.WrapNotFoundError(domain, errG)
	}
...
func GetAllWhere(field string, value any) ([]User, error) {
	dao.CheckDAOReadyState(domain, audit.GET, initialised) // Check the DAO has been initialised, Mandatory.
...
func (record *User) Update(ctx context.Context, note string) error {
	return record.insertOrUpdate(ctx, note, actions.UPDATE.GetCode(), audit.UPDATE, actions.UPDATE.GetCode())
}

func (record *User) UpdateWithAction(ctx context.Context, auditAction audit.Action, note string) error {
	return record.insertOrUpdate(ctx, note, actions.UPDATE.GetCode(), auditAction, actions.UPDATE.GetCode())
}

func (record *User) Create(ctx context.Context, note string) error {
	return record.insertOrUpdate(ctx, note, actions.CREATE.GetCode(), audit.CREATE, actions.CREATE.GetCode())
}

func (record *User) Clone(ctx context.Context) (User, error) {
	return UserClone(ctx, *record)
}

func (record *User) ExportRecordAsJSON(name string) {
...
func ImportRecordsFromCSV() error {
	return importExportHelper.ImportCSV(domain, &User{}, tempalteImportProcessor)
}
